DROP FUNCTION IF EXISTS asset_address_list_wildcard (text, text);

CREATE FUNCTION asset_address_list_wildcard (_asset_policy text, _asset_name text)
  RETURNS TABLE (
    address varchar,
    quantity numeric)
  LANGUAGE PLPGSQL
...
BEGIN
  ...
  WHERE
    ...
    AND MA.name LIKE _asset_name_decoded
